...
export const NewsletterSignup: React.FC<NewsletterSignupProps> = ({ 
  ...
  return (
    <div className={cn(
      ...
      isSidebar 
        ? "bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-6 shadow-sm" 
        : "bg-primary/5 dark:bg-primary/10 p-8 border border-primary/10",
      className
    )}>
      {/* Background Decor */}
      ...
      <div className="relative z-10">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className={cn(
            ...
            isSidebar ? "text-lg text-slate-900 dark:text-white" : "text-xl text-slate-900 dark:text-white"
          )}>
            Newsletter
          </h3>
        </div>
        ...
        <p className={cn(
          ...
          isSidebar ? "text-slate-500 dark:text-slate-400" : "text-slate-600 dark:text-slate-300 max-w-md"
        )}>
          {isSidebar 
        ...
        <form onSubmit={handleSubmit} className="relative group">
          <div className="relative flex items-center">
            <input
              ...
              className={cn(
                "w-full pl-4 pr-12 py-3 rounded-2xl bg-slate-50 dark:bg-slate-800 border transition-all duration-300 outline-none placeholder:text-slate-400 text-sm",
                status === 'error' 
                  ? "border-red-200 focus:border-red-400 ring-2 ring-red-50" 
                  : "border-slate-200 dark:border-slate-700 focus:border-primary/50 focus:ring-4 focus:ring-primary/10"
              )}
            />
            <button
              type="submit"
